...
export class SubService {
  ...
  /**
   * 流式解析订阅内容 - 边下载边解析，减少内存占用
   * 适用于大型订阅文件的高效处理
   */
  private static async parseSubscriptionStream(response: Response): Promise<Proxy[]> {
    ...
    try {
      while (true) {
        ...
        // 检测格式（只检测一次）
        if (!isYamlFormat && !isBase64 && buffer.length > 100) {
          if (buffer.includes('proxies:')) {
            isYamlFormat = true
            yamlContent = buffer
            buffer = ''
            continue
          ...
          // 尝试检测 Base64
          const testStr = buffer.slice(0, 100)
          if (/^[A-Za-z0-9+/=\s]+$/.test(testStr)) {
            isBase64 = true
            base64Buffer = buffer.replace(/\s/g, '')
            buffer = ''
            continue
          }
        }
        ...
        // Base64 格式 - 累积并分批解码
        if (isBase64) {
          base64Buffer += chunk.replace(/\s/g, '')

          // 每收集到足够的数据就尝试解码一批
          if (base64Buffer.length >= 8192) { // 8KB 批次
            try {
              const decoded = Buffer.from(base64Buffer, 'base64').toString()
              ...
              for (const line of lines) {
                ...
                try {
                  const proxy = SingleNodeParser.parse(line.trim())
                  if (proxy) {
                    proxies.push(proxy)
                    // 实时日志进度
                    if (proxies.length % 10 === 0) {
                      logger.debug(`流式解析进度: 已解析 ${proxies.length} 个节点`)
                    }
                  }
                } catch (e) {
                  // 静默失败，继续处理
              ...
              // 重新编码最后一行回 base64
              base64Buffer = Buffer.from(lastLine).toString('base64')
            } catch {
              // 如果解码失败，继续累积
            }
          }
      ...
      // 处理剩余数据
      if (isYamlFormat && yamlContent) {
        try {
          const config = yaml.load(yamlContent) as any
          const yamlProxies = config.proxies || []
          return deduplicateProxies(yamlProxies, { keepStrategy: 'shorter' })
        } catch (e) {
          logger.error('YAML 解析失败:', e)
        }
      }
      ...
      if (buffer.trim()) {
        try {
          const proxy = SingleNodeParser.parse(buffer.trim())
          if (proxy) proxies.push(proxy)
        } catch {
          // 静默失败
        }
      ...
      logger.info(`流式解析完成: 共解析 ${proxies.length} 个节点`)
  ...
  private static async parseSubscriptionWithOriginalMethod(url: string, clientUserAgent?: string): Promise<Proxy[]> {
    ...
    const useStream = !contentLength || parseInt(contentLength) > 102400 // 大于 100KB 使用流式处理

    if (useStream && response.body) {
      logger.info('使用流式处理解析订阅')
      return this.parseSubscriptionStream(response)
    }
