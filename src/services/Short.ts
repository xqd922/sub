...
export class ShortService {
   ...
   * 清理所有短链接 (Sink)
   */
  static async cleanAll(): Promise<{ total: number; deleted: number }> {
    const SINK_URL = process.env.SINK_URL || 'https://link.xqd.us.kg'
    const SINK_TOKEN = process.env.SINK_TOKEN || 'x20030922'
    ...
    logger.debug(`找到 ${allLinks.length} 个短链接`)
    ...
    for (const link of allLinks) {
      try {
        ...
        if (deleteResponse.ok) {
          deletedCount++
          logger.debug(`已删除: ${link.slug}`)
        } else {
          logger.warn(`删除失败 ${link.slug}: ${deleteResponse.status}`)
        }

        // 延迟避免请求过快
        await new Promise(resolve => setTimeout(resolve, 300))
      } catch (error) {
        logger.warn(`删除失败 ${link.slug}: ${error}`)
      }
    }

    logger.debug(`清理完成: ${deletedCount}/${allLinks.length}`)
    return { total: allLinks.length, deleted: deletedCount }
  }
