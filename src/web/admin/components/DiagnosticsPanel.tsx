...
export function DiagnosticsPanel({
  ...
  const healthItems = [
    ...
    },
    {
      ...
      icon: <IconCloud />,
      description:
        "管理 API 与订阅转换接口运行在 Cloudflare Workers。",
    },
    {
      title: "刷新策略",
  ...
  return (
    <Space direction="vertical" size={16} className="admin-section-stack">
      {dataError && <Alert type="error" content={dataError} />}
        ...
        <Col xs={24} lg={8}>
          <Card className="ops-card" title="资源活跃度" bordered>
            ...
            <p className="diagnostic-copy">
              最近 7 天活跃订阅 {stats?.activeRecords ?? 0} 个，总订阅{" "}
              {stats?.totalRecords ?? recordsCount} 个。
            </p>
          </Card>
        </Col>
              ...
              data={[
                {
                  label: "数据状态",
                  value: (
                    <Tag color={dataLoading ? "orange" : "green"}>
                      {dataLoading ? "同步中" : "已同步"}
                    </Tag>
                  ),
                },
          ...
          <Timeline>
            {recentItems.map((item) => (
              <Timeline.Item
                ...
                dot={<IconClockCircle />}
                label={formatAdminDate(item.lastAccess)}
              >
                <Text bold>{item.name}</Text>
                <div>
                  <Text type="secondary">
                    {item.type === "convert" ? "订阅转换" : "短链跳转"} ·{" "}
                    {item.hits.toLocaleString()} 次访问
                  </Text>
                </div>
              </Timeline.Item>
            ))}
