...
import { buildShareLink, formatAdminDate, formatCompactUrl } from "../utils";
...
export function UnifiedTable({
  ...
  const columns: ColumnProps<UnifiedItem>[] = [
      ...
      render: (_, item) => (
        <div className="resource-cell">
          ...
          <Text type="secondary" className="table-subtext">
            {item.type === "convert"
              ? `${item.nodeCount ?? 0} 个节点 · ${item.clientType || "unknown"}`
              : item.id}
          </Text>
        </div>
      ),
    ...
    },
    {
      ...
      render: (hits) => (
        <Text className="number-cell">{Number(hits).toLocaleString()}</Text>
      ),
    },
    {
      title: "最后访问",
      ...
      render: (_, item) => (
        <Text type="secondary" className="date-cell">
          {formatAdminDate(item.lastAccess)}
        </Text>
      ),
    },
    {
  ...
  return (
    <div className="resource-table-card">
      ...
      <Table
        ...
        noDataElement={
          <Empty
            description={searchTerm ? "未找到匹配项" : "暂无管理数据"}
          />
        }
        pagination={{
          sizeCanChange: true,
