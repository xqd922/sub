          ...
          <Card className="ops-card" title="认证配置" bordered>
            <Descriptions
              ...
              data={[
                {
                  label: "默认用户名",
                  value: "ADMIN_USERNAME，未配置时为 admin",
                },
                {
                  label: "管理员密码",
                  value: (
                    <Tag color="green" icon={<IconCheckCircle />}>
                      通过 ADMIN_PASSWORD 配置
                    </Tag>
                  ),
                },
          ...
          <Card className="ops-card" title="运行配置" bordered>
            <Descriptions
              ...
              data={[
                {
                  label: "运行时",
                  value: <Tag color="arcoblue">Cloudflare Workers</Tag>,
                },
                {
                  label: "数据存储",
                  value: (
                    <Tag color="arcoblue" icon={<IconStorage />}>
                      Cloudflare KV / Local Mock
                    </Tag>
                  ),
                },
                ...
                {
                  label: "短链服务",
                  value: "KV → TinyURL → Sink → Bitly 降级链路",
                },
              ]}
            />
          </Card>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card className="ops-card" title="本地开发环境" bordered>
            ...
            <Text type="secondary">
              本地使用 .dev.vars 配置，文件已被 .gitignore 忽略，不会提交到仓库。
            </Text>
          </Card>
        </Col>
          ...
          <Card className="ops-card" title="安全建议" bordered>
            <Space direction="vertical" size={10}>
              <div className="security-tip">
                <IconLock /> 生产环境必须设置强密码。
              </div>
              <div className="security-tip">
                <IconSafe />{" "}
                不要将 .dev.vars、token 或 KV 凭据提交到 Git。
              </div>
              <div className="security-tip">
                <IconCheckCircle />{" "}
                修改 ADMIN_PASSWORD 后，旧登录 token 会自然失效。
              </div>
            </Space>
          </Card>
        </Col>
