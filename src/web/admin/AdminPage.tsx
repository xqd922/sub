...
import { buildAdminItems, formatAdminDate } from "./utils";
...
export function AdminPage() {
  ...
  const tableItems = useMemo(
    () => buildAdminItems(records, shortLinks),
    [records, shortLinks],
  );
                    ...
                    <Space size={8}>
                      <div className="distribution-track">
                        <div
                          ...
                          style={{
                            width: `${Math.max(12, (count / records.length) * 100)}%`,
                          }}
                        />
                      </div>
        ...
        <Col xs={24} lg={8}>
          <Card className="ops-card" title="高访问资源" bordered>
            <Space direction="vertical" size={10} className="full-width">
              ...
              {topItems.length === 0 && (
                <Text type="secondary">暂无访问数据</Text>
              )}
            </Space>
          </Card>
        </Col>
  ...
  const renderContent = () => {
    ...
    if (activeSection === "settings")
      return <SettingsPanel autoRefresh={autoRefresh} token={token} />;
