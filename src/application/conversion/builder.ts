...
import { Proxy } from '@/src/domain/proxy/types'
import { defaultConfig, generateProxyGroups } from '@/src/presentation/config/clash'
import { generateSingboxConfig as buildSingboxConfig } from '@/src/presentation/config/singbox'
import { generateBase64Subscription } from '@/src/domain/subscription/protocols'
import { previewStyles } from '@/src/presentation/preview/styles'
...
export class ConfigService {
  ...
  static generateClashConfig(proxies: Proxy[], isAirportSubscription: boolean = true): string {
    ...
    const clashProxies = proxies.map(proxy => {
      const { detour: _detour, ...clashProxy } = proxy
      return clashProxy
    })
