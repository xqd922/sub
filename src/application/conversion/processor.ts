import { Proxy, YamlSubscription } from '@/src/domain/proxy/types'
import { SingleNodeParser } from '@/src/domain/subscription/node-parser'
import { fetchNodesFromRemote } from '@/src/domain/subscription/remote-source'
import { formatProxies as formatProxiesImpl } from '@/src/domain/proxy/format'
import { isProtocolUrl, isGistUrl, shouldFormatNodeNames } from '@/src/domain/proxy/protocols'
import { NetService } from '@/src/infrastructure/network/client'
import { logger } from '@/src/infrastructure/logger'
import { formatBytes } from '@/src/shared/utils'
import { deduplicateProxies } from '@/src/domain/proxy/dedup'
...
export class SubService {
  ...
  private static async parseSubscriptionWithOriginalMethod(url: string, clientUserAgent?: string): Promise<Proxy[]> {
    // 使用原来的网络请求方法
    const response = await NetService.fetchWithRetry(url, { userAgent: clientUserAgent })
